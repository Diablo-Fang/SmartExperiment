...
//                      browserWnd.z=4;
                        ...
                        mainloader.item.show()
                        mainloader.item.requestActivate()
                 } else if(qurl === "ScpiTestWindow.qml") {
                        mainloader.source = "ScpiTestWindow.qml"
                        if (mainloader.item) {
                            // ScpiTestWindow's root is Rectangle, it doesn't have a show() method like Popup/Window.
                            // The Loader handles its visibility once the source is set.
                            // requestActivate() is typically for Window types.
                            // We will try to call them if they exist, similar to other items.
                            if (typeof mainloader.item.show === 'function') {
                                mainloader.item.show();
                            } else {
                                // For a Rectangle, ensuring it's visible is usually enough if the loader is visible.
                                // mainloader.item.visible = true; // This should be default true for the Rectangle
                            }
                            if (typeof mainloader.item.requestActivate === 'function') {
                                mainloader.item.requestActivate();
                            }
                            console.log("Attempting to load ScpiTestWindow.qml into mainloader.");
                        } else {
                            console.error("Failed to set source for ScpiTestWindow.qml or mainloader.item is initially null after source set.");
                        }
                 } else if(qurl === "workpage.qml") {
                        mainloader.source = "workpage.qml"
                        if (mainloader.item) {
                            // 假设 workpage.qml 的根元素也是一个可以 show/requestActivate 的类型
                            // 如果 workpage.qml 的根元素是 Page, Rectangle 等，则 show() 可能不是必需的或不存在
                            // Loader 会自动处理其内容的加载和显示
                            if (typeof mainloader.item.show === 'function') {
                                mainloader.item.show();
                            }
                            if (typeof mainloader.item.requestActivate === 'function') {
                            ...
                            console.log("Attempting to load workpage.qml into mainloader.");
                        } else {
                            console.error("Failed to set source for workpage.qml or mainloader.item is initially null after source set.");
                        }
                 }
            }
